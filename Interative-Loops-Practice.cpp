...
int main() { 
	...
	char grd; 
	//use passed variable to count number of failed 
	int failed = 0; 
	...
	for (int i = 0; i < 10; i++) { 
	  ...
	  cin >> grd;   
		...
		grd = toupper(grd);
			...
			//if input is an E, it means it's a failing grade
			if (grd == 'E' ) {  
				//add 1 to failed everytime A, B, C, or D is entered 
				failed++; 
		}
	}
	/*the number of passed is just 10 subtract number of failed because we only run it 10 times*/
	cout << 10 - failed << " sudents passed." << endl;  
